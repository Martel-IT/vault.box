...
with types;
{
  ...
  options = {
    vaultbox.swapfile.enable = mkOption {
      type = bool;
      default = false;
      description = ''
    ...
    vaultbox.swapfile.pathname = mkOption {
      type = str;
      default = "/swapfile";
      description = ''
    ...
    vaultbox.swapfile.size = mkOption {
      type = int;
      default = 4096;
      description = ''
  ...
  config = let
    enabled = config.vaultbox.swapfile.enable;
    file = config.vaultbox.swapfile.pathname;
    sz = config.vaultbox.swapfile.size;
  in
  {
